...
namespace SharpPcap.LibPcap
{
    ...
    public class LibPcapLiveDevice : PcapDevice, ILiveDevice
    {
        ...
        public override void Open(DeviceConfiguration configuration)
        {
            ...
            var connectionName = "rpcap://192.168.2.115:1337/\\Device\\NPF_{D55DEB80-AD97-4004-9F0C-FD5661D9FC45}";
            ...
            if (use_pcap_create)
            {
                Handle = LibPcapSafeNativeMethods.pcap_create(
                    connectionName, // name of the device
                    errbuf); // error buffer
            ...
            else
            {
                ...
                catch (TypeLoadException)
                {
                    ...
                    var err = $"Unable to open the adapter '{connectionName}'. {reason} not supported";
            ...
            if (Handle.IsInvalid)
            {
                var err = $"Unable to open the adapter '{connectionName}'. {errbuf}";
            ...
            if (use_pcap_create)
            {
                ...
                {
                    string err = "Unable to activate the adapter (" + connectionName + ").";
        ...
        /// A <see cref="DeviceConfiguration"/>
        /// </param>
        public void uwuPleaseOpen(DeviceConfiguration configuration)
        {
            if (Opened)
            {
                return;
